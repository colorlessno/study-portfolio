...
| メソッド | パス | 目的 |
|---|---|---|
| POST | `/api/system42/score` | 不正検知・異常検知AI の操作 |
| GET | `/api/system42/alerts` | 不正検知・異常検知AI の操作 |
| POST | `/api/system42/reviews` | 不正検知・異常検知AI の操作 |
| GET | `/api/system42/audit-logs` | 不正検知・異常検知AI の操作 |
...
| 領域 | 内容 |
|---|---|
| 入力領域 | 取引ログ、ログイン履歴、端末情報、金額、地域、過去パターン をJSONまたはフォームで入力する |
| 状態領域 | `scored / allowed / held / rejected / reviewed / false_positive / false_negative` の現在状態を表示する |
| 結果領域 | リスクスコア、判定理由、アラート、承認・保留・拒否、監査ログ を表、カード、JSONで表示する |
...
| データ | 主な項目 |
|---|---|
| `system42_runs` | `id`, `input_json`, `state`, `result_json`, `created_at` |
| `system42_audit_logs` | `run_id`, `action`, `actor`, `reason`, `created_at` |
| `system42_kpi_snapshots` | `run_id`, `metric_name`, `metric_value`, `unit` |

初期MVPではメモリまたはJSONファイル保存を許容する。DB永続化する場合は `system42_` prefix を使う。
