...
category/StudyDB\src\apps\common\scripts\run-sql.cmd db04 sql\001_schema.sql
category/StudyDB\src\apps\common\scripts\run-sql.cmd db04 sql\002_seed.sql
category/StudyDB\src\apps\common\scripts\run-sql.cmd db04 sql\003_commit_rollback.sql
```
...
```cmd
docker compose -f category/StudyDB\src\apps\common\docker-compose.yml exec db psql -U postgres -d studydb
```
...
自動検証は `node category/StudyDB\scripts\validate-studydb.mjs db04`、環境の停止は `docker compose -f category/StudyDB\src\apps\common\docker-compose.yml down` です。
