...
| pattern | 推薦・ランキング |
| default_input | ユーザー属性、行動ログ、候補アイテム、実験条件 を含む教材用JSON |
| state_flow | logged / ranked / displayed / clicked / converted / evaluated |
| kpi_definitions | クリック率、コンバージョン率、A/B差分、レイテンシ_ms |
| risk_points | 個人属性に基づく不当な推薦、フィルターバブル、推薦偏向 |
...
| state | string | logged / ranked / displayed / clicked / converted / evaluated のいずれか |
| result.summary | string | 判断概要 |
| result.recommendations | array | 推奨、候補、検知、最適化案など |
| result.explanations | array | 判断理由 |
| result.risk_flags | array | 個人属性に基づく不当な推薦、フィルターバブル、推薦偏向 に基づく注意点 |
| audit_log | array | 入力受付、判断、fallback、完了の証跡 |
| kpi_snapshot | object | クリック率、コンバージョン率、A/B差分、レイテンシ_ms を含むKPI |

## 6. 状態遷移

| from | event | to | 監査ログ |
|---|---|---|---|
| start | request accepted | logged | request_received |
| logged | mock decision completed | ranked | decision_generated |
| ranked | risk found | displayed | risk_flagged |
| ranked | no blocking risk | converted | execution_completed |
| any | LM Studio unavailable | current state | lmstudio_fallback_to_mock |

system38 の状態候補は logged / ranked / displayed / clicked / converted / evaluated とし、画面では現在状態、次状態、終了状態を表示する。
...
| KPI | 内容 |
|---|---|
| クリック率 / コンバージョン率 / A/B差分 / レイテンシ_ms | system38 の主要KPI |
...
python .\src\scripts\system38_enterprise_demo.py
