...
class TextExtractor:
    ...
    def validate_file_name(self, file_name: str | None) -> str:
        if not file_name:
            raise ValidationAppError("invalid_document_file", "文書ファイル名が不正です。")
        suffix = Path(file_name).suffix.lower()
        if suffix not in self.SUPPORTED_EXTENSIONS:
            raise ValidationAppError("invalid_document_file", "対応していない文書形式です。")
        return suffix
    ...
    def _extract_pdf(self, file_bytes: bytes) -> str:
        try:
            ...
            raise ValidationAppError("pdf_support_missing", "PDF抽出ライブラリが利用できません。") from exc
    ...
    def _extract_docx(self, file_bytes: bytes) -> str:
        with ZipFile(BytesIO(file_bytes)) as archive:
            try:
                ...
                raise ValidationAppError("invalid_document_file", "DOCX本文が見つかりません。") from exc
    ...
    def _decode_text(self, file_bytes: bytes) -> str:
        for encoding in ("utf-8-sig", "utf-8", "cp932"):
            try:
                ...
                continue
        raise ValidationAppError("invalid_document_encoding", "文書テキストを復号できませんでした。")
