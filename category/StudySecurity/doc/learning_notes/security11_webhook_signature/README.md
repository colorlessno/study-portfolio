...
```powershell
npm.cmd --prefix category/StudySecurity/src/backend/src/studysecurity/systems/security11_webhook_signature run check
npm.cmd --prefix category/StudySecurity/src/backend/src/studysecurity/systems/security11_webhook_signature test
npm.cmd --prefix category/StudySecurity/src/backend/src/studysecurity/systems/security11_webhook_signature run demo
```
...
HTTP serverを確認する場合は`npm.cmd --prefix category/StudySecurity/src/backend/src/studysecurity/systems/security11_webhook_signature run start`で`http://localhost:4111/webhook`を起動し、確認後に`Ctrl+C`で停止します。最初は署名生成が内包されたCLI demoから始めます。
