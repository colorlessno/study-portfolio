...
| 区分 | 内容 |
|---|---|
| 入力 | 希望条件、候補リスト、空き枠、価格、本人確認状況 |
| 出力 | 条件整理・候補一覧、推奨理由、確定確認、実行結果 |
| 状態 | hearing / proposed / confirming / executed / changed / cancelled / escalated |

## 7. API設計

| メソッド | パス | 目的 |
|---|---|---|
| POST | `/api/system37/sessions` | 取引実行型AIコンシェルジュ の操作 |
| POST | `/api/system37/proposals` | 取引実行型AIコンシェルジュ の操作 |
| POST | `/api/system37/confirm` | 取引実行型AIコンシェルジュ の操作 |
| POST | `/api/system37/cancel` | 取引実行型AIコンシェルジュ の操作 |
...
| 領域 | 内容 |
|---|---|
| 入力領域 | 希望条件、候補リスト、空き枠、価格、本人確認状況 をJSONまたはフォームで入力する |
| 状態領域 | `hearing / proposed / confirming / executed / changed / cancelled / escalated` の現在状態を表示する |
| 結果領域 | 条件整理・候補一覧、推奨理由、確定確認、実行結果 を表、カード、JSONで表示する |
...
| データ | 主な項目 |
|---|---|
| `system37_runs` | `id`, `input_json`, `state`, `result_json`, `created_at` |
| `system37_audit_logs` | `run_id`, `action`, `actor`, `reason`, `created_at` |
| `system37_kpi_snapshots` | `run_id`, `metric_name`, `metric_value`, `unit` |

初期MVPではメモリまたはJSONファイル保存を許容する。DB永続化する場合は `system37_` prefix を使う。
