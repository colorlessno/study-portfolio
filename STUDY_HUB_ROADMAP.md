...
4. ユーザー利用シナリオを3〜5件に絞る。
5. 既存テーマを10件だけ選び、確定したインターフェースでメタデータ試作を行う。
6. `THEME_CATALOG.md`との正本関係を決める。
7. DBを使わないPhase 1の画面モックを確認する。
8. 承認後にアプリのディレクトリと技術構成を確定する。

1〜3はStudy Hub立ち上げ前の実装ゲートとします。物理ディレクトリ分割は必須条件にせず、単体利用契約と安全な後片付けを満たせるかで判定します。
