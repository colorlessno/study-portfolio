...
| メソッド | パス | 目的 |
|---|---|---|
| POST | `/api/system41/images` | コンピュータビジョン / マルチモーダルAI の操作 |
| POST | `/api/system41/detect` | コンピュータビジョン / マルチモーダルAI の操作 |
| GET | `/api/system41/review-items` | コンピュータビジョン / マルチモーダルAI の操作 |
| POST | `/api/system41/reviews` | コンピュータビジョン / マルチモーダルAI の操作 |
...
| 領域 | 内容 |
|---|---|
| 入力領域 | 棚画像、レシート画像、商品リスト、センサーイベント、OCR結果 をJSONまたはフォームで入力する |
| 状態領域 | `uploaded / detected / matched / review_required / confirmed / rejected` の現在状態を表示する |
| 結果領域 | 検出物品、数量推定、異常候補、信頼度、確認待ち一覧 を表、カード、JSONで表示する |
...
| データ | 主な項目 |
|---|---|
| `system41_runs` | `id`, `input_json`, `state`, `result_json`, `created_at` |
| `system41_audit_logs` | `run_id`, `action`, `actor`, `reason`, `created_at` |
| `system41_kpi_snapshots` | `run_id`, `metric_name`, `metric_value`, `unit` |

初期MVPではメモリまたはJSONファイル保存を許容する。DB永続化する場合は `system41_` prefix を使う。
