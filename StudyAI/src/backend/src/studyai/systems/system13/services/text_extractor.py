...
class TextExtractor:
    ...
    def validate_file_name(self, file_name: str | None) -> str:
        if not file_name:
            raise ValidationAppError("invalid_knowledge_file", "A file name is required.")
        suffix = Path(file_name).suffix.lower()
        if suffix not in self.SUPPORTED_EXTENSIONS:
            raise ValidationAppError("invalid_knowledge_file", "The file type is not supported.")
        return suffix
    ...
    def _extract_pdf(self, file_bytes: bytes) -> str:
        try:
            ...
            raise ValidationAppError("pdf_support_missing", "PDF support library is missing.") from exc
    ...
    def _extract_docx(self, file_bytes: bytes) -> str:
        with ZipFile(BytesIO(file_bytes)) as archive:
            try:
                ...
                raise ValidationAppError("invalid_knowledge_file", "DOCX content could not be read.") from exc
    ...
    def _extract_xlsx(self, file_bytes: bytes) -> str:
        try:
            ...
            raise ValidationAppError("xlsx_support_missing", "Excel support library is missing.") from exc
    ...
    def _decode_text(self, file_bytes: bytes) -> str:
        for encoding in ("utf-8-sig", "utf-8", "cp932"):
            try:
                ...
                continue
        raise ValidationAppError("invalid_knowledge_encoding", "The file encoding is not supported.")
