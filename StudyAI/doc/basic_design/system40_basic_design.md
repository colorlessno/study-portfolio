...
| メソッド | パス | 目的 |
|---|---|---|
| POST | `/api/system40/forecast` | 需要予測・在庫最適化AI の操作 |
| GET | `/api/system40/risks` | 需要予測・在庫最適化AI の操作 |
| POST | `/api/system40/replenishment` | 需要予測・在庫最適化AI の操作 |
| POST | `/api/system40/approvals` | 需要予測・在庫最適化AI の操作 |
...
| データ | 主な項目 |
|---|---|
| `system40_runs` | `id`, `input_json`, `state`, `result_json`, `created_at` |
| `system40_audit_logs` | `run_id`, `action`, `actor`, `reason`, `created_at` |
| `system40_kpi_snapshots` | `run_id`, `metric_name`, `metric_value`, `unit` |

初期MVPではメモリまたはJSONファイル保存を許容する。DB永続化する場合は `system40_` prefix を使う。
