...
| result.risk_flags | array | 個人情報の過剰収集、正常取引の誤ブロック、監査ログ改ざん に基づく注意点 |
...
| from | event | to | 監査ログ |
|---|---|---|---|
| start | request accepted | scored | request_received |
| scored | mock decision completed | held | decision_generated |
| held | risk found | reviewed | risk_flagged |
| held | no blocking risk | allowed | execution_completed |
...
| error_code | HTTP | 条件 |
|---|---|---|
| system42_not_found | 404 | catalog に system定義がない |
| system42_input_invalid | 400 | input がobject ではない |
| system42_unsafe_input_masked | 200 | 秘密情報相当の入力を mask して続行した |
| system42_execution_failed | 500 | 想定外例外 |
...
python .\src\scripts\system42_enterprise_demo.py
