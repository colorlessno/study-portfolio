...
| メソッド | パス | 目的 |
|---|---|---|
| POST | `/api/system43/problems` | 制約最適化AI の操作 |
| POST | `/api/system43/optimize` | 制約最適化AI の操作 |
| POST | `/api/system43/adjustments` | 制約最適化AI の操作 |
| GET | `/api/system43/results` | 制約最適化AI の操作 |
...
| データ | 主な項目 |
|---|---|
| `system43_runs` | `id`, `input_json`, `state`, `result_json`, `created_at` |
| `system43_audit_logs` | `run_id`, `action`, `actor`, `reason`, `created_at` |
| `system43_kpi_snapshots` | `run_id`, `metric_name`, `metric_value`, `unit` |

初期MVPではメモリまたはJSONファイル保存を許容する。DB永続化する場合は `system43_` prefix を使う。
