...
```powershell
node StudyAWS\scripts\validate-studyaws.mjs aws02
```

Docker Desktopがある場合だけ、リポジトリルートから追加観察します。

```powershell
docker compose -f StudyAWS\src\backend\src\studyaws\systems\aws02_security_group_port\docker-compose.yml up -d
Invoke-RestMethod http://localhost:4102
docker compose -f StudyAWS\src\backend\src\studyaws\systems\aws02_security_group_port\docker-compose.yml exec -T web node -e "fetch('http://api:5102').then(r => r.text()).then(console.log)"
docker compose -f StudyAWS\src\backend\src\studyaws\systems\aws02_security_group_port\docker-compose.yml down
```
